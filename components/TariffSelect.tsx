  ...
  return (
    <div ref={rootRef} className={`relative ${isBanana ? "w-full sm:w-auto" : "w-full"}`}>
      ...
      <button
        ...
        className={
          isInverted
            ? "flex h-10 w-full items-center justify-between gap-2 rounded-md border border-neutral-800 bg-[#111111] px-2.5 text-left text-sm text-neutral-100 outline-none transition-colors hover:bg-neutral-900 dark:border-neutral-300 dark:bg-white dark:text-[#111111] dark:hover:bg-neutral-100 sm:h-8 sm:min-w-[12rem] sm:w-auto"
            : isBanana
      ...
      >
        <span className="flex min-w-0 flex-1 items-center gap-2">
          <span className={isDarkSurface ? "truncate font-medium" : "truncate text-foreground"}>
            {selected.label}
          </span>
      ...
      {open ? (
        <div
          className={`absolute z-30 mt-2 overflow-hidden rounded-md ${
            ...
            isInverted
              ? "w-[min(18rem,calc(100vw-2rem))] border border-neutral-800 bg-[#111111] dark:border-neutral-300 dark:bg-white"
              : isBanana
