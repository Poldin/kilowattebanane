...
function OfferDettaglio({ hit }: { hit: OfferteSearchHit }) {
  const d = hit.dettaglio;
  const rows = dettaglioRows(hit);
  const hasBody =
    d.descrizione ||
    d.garanzie ||
    rows.length > 0 ||
...
function dettaglioRows(hit: OfferteSearchHit) {
  const d = hit.dettaglio;
  ...
  const potenza = formatBound(
    d.potenzaMin,
    d.potenzaMax,
    "kW",
    (value) => formatPotenzaKw(value),
  );
...
function OfferCodeLink({ hit }: { hit: OfferteSearchHit }) {
  const vendorHref = absoluteUrl(hit.urlOfferta);
...
function VendorName({ hit }: { hit: OfferteSearchHit }) {
  const href = absoluteUrl(hit.urlOfferta ?? hit.urlVenditore);
  if (!href) {
...
function absoluteUrl(raw: string | null) {
  if (!raw) return null;
